...
function SummaryCard({ theme, t, summary }) {
  const items = [
    {
      key: "sessions",
      label: t.sessionsLogged ?? "Sessions",
      ...
      <View style={s.summaryGrid}>
        {items.map((it) => (
          <View key={it.key} style={s.summaryCell}>
            ...
            <Text
              style={[s.summaryLabel, { color: theme.textMuted }]}
              numberOfLines={1}
            >
              {it.label}
            </Text>
...
function ChartCard({ theme, t, title, subtitle, height = 220, children }) {
  return (
    ...
    >
      ...
      {subtitle ? (
        <Text style={[s.cardSubtitle, { color: theme.textMuted }]}>
          {subtitle}
        </Text>
      ) : null}
...
export default function MyDataScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    scoresApi
      ...
      .then((arr) => {
        ...
        // If you're seeing "no scores", uncomment this to inspect:
        // console.log("[scoresApi.list] returned", list.length, "items, sample:", list[0]);

        const map = {};
        for (const sc of list) {
          if (sc && sc.date && typeof sc.compositeScore === "number") {
            map[sc.date] = sc.compositeScore;
          }
        }
        setScoresByDate(map);
      })
      .catch((err) => {
        // Don't fail silently — log so you know if the API is broken.
        console.warn("[scoresApi.list] failed:", err?.message ?? err);
        setScoresByDate({});
      });
  }, [fetchLogs]);
  ...
  const heatmap = useMemo(
    () => heatmapCells(logs || [], scoresByDate, 26),
    [logs, scoresByDate],
  );
  const weekly = useMemo(
    () =>
      weeklyVolume(logs || [], 12).map((w) => ({
        ...w,
    ...
    <View style={[s.root, { backgroundColor: theme.bg }]}>
      ...
      <ScrollView
        ...
        contentContainerStyle={[
          s.scrollContent,
          { paddingBottom: (insets.bottom || 12) + Spacing.xl },
        ]}
        ...
        {loading && !logs ? (
          ...
          <>
            ...
            <ChartCard
              ...
              subtitle={
                t.last30days ?? "Strength / cardio / mobility / recovery"
              }
              height={300}
            >
              <DonutChart
                slices={cats}
                theme={theme}
                totalLabel={(t.totalHours ?? "HOURS").toUpperCase()}
              />
            </ChartCard>
