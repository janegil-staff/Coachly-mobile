...
const HEIGHTS = Array.from({ length: 101 }, (_, i) => String(i + 100));
const WEIGHTS = Array.from({ length: 181 }, (_, i) => String(i + 20));
...
function Field({ label, value, onChangeText, keyboardType, theme }) {
  return (
    <View style={{ width: "100%", marginBottom: Spacing.lg }}>
      <Text style={{ color: theme.textSecondary, fontSize: FontSize.md, fontWeight: "600", marginBottom: 6 }}>{label}</Text>
      <TextInput
...
function Dropdown({ label, value, options, onSelect, unit, theme }) {
  ...
  return (
    <View style={{ flex: 1 }}>
      <Text style={{ color: theme.textSecondary, fontSize: FontSize.sm, fontWeight: "600", marginBottom: 6 }}>{label}</Text>
      <TouchableOpacity style={[s.dropTrigger, open && { borderColor: theme.accent }]} onPress={() => setOpen(true)} activeOpacity={0.8}>
...
export default function RegisterScreen({ navigation, route }) {
  ...
  const advanceToStep2 = async () => {
    ...
    if (email.trim().toLowerCase() !== emailConfirm.trim().toLowerCase()) {
      setError(t.emailMismatch ?? "Emails do not match"); return;
    }
    ...
    try {
      ...
      if (exists) { setError(t.emailAlreadyExists ?? "Email already registered"); return; }
    } catch (e) {
      setError(e?.message ?? "Could not verify email"); return;
    } finally {
      setLoading(false);
    }
    setStep(2);
    ...
    <View style={s.bg}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
          ...
          {step === 1 && (
            <>
              ...
              <View style={{ marginBottom: Spacing.xl }}>
                <TouchableOpacity style={[s.dropdown, langOpen && { borderColor: theme.accent }]} onPress={() => setLangOpen((o) => !o)} activeOpacity={0.8}>
                    ...
                    {LANGUAGES.map(({ code, flag, label }) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <Text style={[s.dropdownItemLabel, { color: active ? theme.accent : theme.text }, active && { fontWeight: "700" }]}>
                            {label}
                          </Text>
                          {active && <Text style={{ color: theme.accent, fontSize: 16 }}>✓</Text>}
                        </TouchableOpacity>
          ...
          {step === 2 && (
            <>
              <Image source={require("../../../assets/images/register_header.png")} style={s.headerImage} resizeMode="cover" />
